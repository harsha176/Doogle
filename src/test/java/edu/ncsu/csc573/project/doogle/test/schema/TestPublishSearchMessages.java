...
public class TestPublishSearchMessages {
    ...
    /*
	private IRequest getPublishRequest() {
	...
	private IRequest testPublishFolderRequest() {
	...
	/*@Test
	public void testPublishRequestMessageParseXML() {
		...
		PublishSearchParameter pubSearchParam = (PublishSearchParameter) publishReq.getParameter();
		pubSearchParam.resetCounter();
		int i = 0;
		//while(pubSearchParam.getParamCount() < pubSearchParam.getSize()) {
			Assert.assertEquals(pubSearchParam.getParamValue(EnumParamsType.DELIMITER), i);
			Assert.assertEquals(pubSearchParam.getParamValue(EnumParamsType.FILENAME), "abc.txt");
			pubSearchParam.setNextParam();
			Assert.assertEquals(pubSearchParam.getParamValue(EnumParamsType.DELIMITER), 1);
			Assert.assertEquals(pubSearchParam.getParamValue(EnumParamsType.FILENAME), "xyz.txt");
			pubSearchParam.setNextParam();
			Assert.assertEquals(pubSearchParam.getParamValue(EnumParamsType.DELIMITER), 2);
			Assert.assertEquals(pubSearchParam.getParamValue(EnumParamsType.IPADDRESS), "10.12.2.4");
