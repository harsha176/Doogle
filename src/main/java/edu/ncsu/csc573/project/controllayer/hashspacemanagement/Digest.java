...
public class Digest {
	...
	public static final int NUM_BITS_INTEGER = 32;
	...
	public boolean get_digest_for_file(String filename, char[] arr) {
		...
		if(fd.isFile()) {
			logger.error("Failed to open File " + filename +" for creating digest\n");
			return false;
		} } catch (Exception e) {
			logger.error("Failed to open File " + filename +" for creating digest\n");
			return false;
		...
		try {
			while((buffer = is.read()) != -1) {
				if(!can_include_char_in_digest((char)buffer)){
					continue;
				} 
				/*New line or a space */ 
				if(stop_digest_this_word((char)buffer)){
					...
					if(master == true){
						...
						arr[tmp_digest/NUM_BITS_INTEGER]=(char) (arr[tmp_digest/NUM_BITS_INTEGER]|(1<<(tmp_digest%NUM_BITS_INTEGER)));
					}
					numRead = 0;		
	 ...
	 */
	public boolean get_digest_for_query(String query,  char[] arr) {
	 ...
	 * Else Returns 0
	 */
	boolean can_include_char_in_digest(char ch){
	...
	boolean stop_digest_this_word(char ch) {
		if(ch == ' ' || ch == '\n'){
			return true;	
		}
		return false;
